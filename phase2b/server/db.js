...
function runMigrations() {
  ...
  // We only rebuild if we detect the old constraint.
  try {
...
function migrateExpandSectionCheck() {
  // Tables that have a CHECK(section IN ('home','work')) constraint to expand.
  // Because of foreign keys + WAL, we do it all in a single transaction with FK deferred.
  ...
  const tx = db.transaction(() => {
    for (const t of tablesToRebuild) {
      ...
      // Build new CREATE with inbox included.
      ...
      // Rename original, create new with expanded check, copy data, drop old.
      db.exec(`ALTER TABLE ${t} RENAME TO ${t}__old_migration;`);
      // The renamed CREATE still has the old name in its SQL. We constructed newSql for the
      // original name, which is now available again.
      db.exec(newSql);
      const cols = db.prepare(`PRAGMA table_info(${t})`).all().map(c => c.name).join(',');
      db.exec(`INSERT INTO ${t} (${cols}) SELECT ${cols} FROM ${t}__old_migration;`);
      db.exec(`DROP TABLE ${t}__old_migration;`);
