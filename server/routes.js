...
// Archived tasks - MUST be before :id route
...
// Weekly review - MUST be before :id route
...
router.post('/tasks', authenticate, (req, res) => {
  const { section, title, description, status, priority, due_date, case_reference, client_name, court_date, goals } = req.body;
  if (!section || !title) return res.status(400).json({ error: 'Section and title required' });
  const db = getDb();
  const result = db.prepare('INSERT INTO tasks (user_id,section,title,description,status,priority,due_date,case_reference,client_name,court_date,goals) VALUES (?,?,?,?,?,?,?,?,?,?,?)').run(
    req.user.id, section, title, description || '', status || 'to_start', priority || 3, due_date || null, case_reference || null, client_name || null, court_date || null, goals || ''
  );
...
router.put('/tasks/:id', authenticate, (req, res) => {
  ...
  const fields = ['title', 'description', 'status', 'priority', 'due_date', 'pinned', 'archived', 'sort_order', 'case_reference', 'client_name', 'court_date', 'goals'];
...
// Recent activity across all sections for the current user
...
router.get('/preferences', authenticate, (req, res) => {
  ...
  res.json({ preferences: pref || { theme: 'dark' } });
});
...
router.put('/preferences', authenticate, (req, res) => {
  ...
  if (ex) {
    db.prepare('UPDATE user_preferences SET theme = ? WHERE user_id = ?').run(theme || 'dark', req.user.id);
  } else {
...
router.get('/export/csv', authenticate, (req, res) => {
  ...
  // attach tags as comma-separated names
