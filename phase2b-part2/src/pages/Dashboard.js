...
function CreateTaskModal({ section, onClose, onCreated, prefillTemplate }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (prefillTemplate && prefillTemplate.id) {
        // Instantiate the template, then (if user edited fields) apply overrides via the instantiate payload.
        ...
        created = d.task;
        // If the user changed status/court_date, patch separately (instantiate endpoint ignores these).
        if (form.status !== 'to_start' || form.court_date) {
          try {
  ...
  return (
    <div className="dash-modal-overlay" onClick={onClose}><div className="dash-modal" onClick={e => e.stopPropagation()}>
      <h2>{prefillTemplate ? `📋 ${prefillTemplate.name}` : `New ${section === 'work' ? 'Work' : section === 'inbox' ? 'Inbox' : 'Home'} Task`}</h2>
...
export default function Dashboard({ section, onBack, theme, onToggleTheme, pendingOpenTask, onPendingHandled, onOpenPomodoro }) {
  const toast = useToast();
  ...
  const [viewMode, setViewMode] = useState('list'); // 'list' | 'board' | 'matrix' | 'calendar'
  ...
  // Handle pending task from global search
  useEffect(() => {
    if (pendingOpenTask) {
  ...
  // Dashboard-level keyboard shortcuts: n (new), e (edit selected), d (done), p (pin), a (archive), j/k (navigate)
  // Global shortcuts (/, Shift+N, ?, Ctrl+K, Esc) are handled in App.js.
  useEffect(() => {
    const isTyping = (e) => {
      const el = e.target;
      if (!el) return false;
    ...
    const handler = async (e) => {
      ...
      if (modal || detailId) return; // don't hijack when a modal is open
      const list = filtered; // source of truth for navigation
  ...
  return (
    <div className={`dashboard dashboard--${section}${bulkSel.size > 0 ? ' dash-bulk-active' : ''}`}>
      <div className="dash-header">
        <div className="dash-header-left"><button className="dash-back-btn" onClick={onBack}>←</button><div className="dash-title"><span className="dash-title-icon">{section==='work'?'💼':section==='inbox'?'📥':'🏠'}</span>{section==='work'?'Work':section==='inbox'?'Inbox':'Home'}</div></div>
        ...
        <div className="dash-header-right">
          <ViewSwitcher value={viewMode} onChange={setViewMode} section={section} />
          <SavedViewsMenu
            section={section}
      ...
      {modal==='task'&&<CreateTaskModal section={section} onClose={()=>{setModal(null);setPendingTemplate(null);}} onCreated={()=>loadT()} prefillTemplate={pendingTemplate}/>}
      {templatePickerOpen && (
      ...
      {templateMgrOpen && (
        <TemplateManager section={section} onClose={() => setTemplateMgrOpen(false)} onChanged={() => {}} />
      )}
